#!/usr/bin/env python3
"""
gen_article_api.py — 调用硅基流动 DeepSeek V3.2 生成SEO文章（完整版）
...
def make_tool_link(slug, name):
    """生成工具内链Markdown"""
    return f"[{name}](https://www.aitoolbox.hk/tools/{slug}/index.html)"
...
def validate_links(content, expected_links):
    """验证文章中的内链数量和质量"""
    link_pattern = r'\[([^\]]+)\]\(https://www\.aitoolbox\.hk/tools/([^/]+)/index\.html\)'
...
def quality_check(article):
    ...
    type_info = ARTICLE_TYPES.get(article.get('_type', 'B'), ARTICLE_TYPES['B'])
    min_w, max_w = 1500, 4000
    wc_range = type_info['word_count']
    if '-' in wc_range:
        parts = wc_range.split('-')
        min_w, max_w = int(parts[0]), int(parts[1])

    if min_w <= word_count <= max_w:
        passes.append(f"字数: {word_count}字（要求{wc_range}）")
    else:
        warnings.append(f"字数: {word_count}字（要求{wc_range}）")
...
字数要求：{type_info['word_count']}字
具体要求：
{requirements_str}

## 内链要求（必须遵守）
文章中必须自然地插入3-5个工具内链。内链必须在正文中自然出现，不是硬塞的。
格式：[工具名](https://www.aitoolbox.hk/tools/工具slug/index.html)
本篇可用内链工具：{link_md}
...
请直接输出Markdown格式的文章（从#标题开始），不需要输出slug、日期等元数据。"""
...
请直接输出Markdown格式的文章（从#标题开始），不需要输出slug、日期等元数据。"""
    ...
    print(f"  主题: {topic}")
    print(f"  类型: {type_info['label']} ({type_info['word_count']}字)")
