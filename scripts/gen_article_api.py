...
- [ ] 是否引用了1-2个外部权威来源？（官方文档、知名评测、行业报告，增强GEO跨平台信任）
"""
...
def generate_article(topic, article_type, existing_titles, seo_keywords=None):
    ...
    system_prompt = f"""你是AI工具评测博主，运营网站www.aitoollab.cn。你每天使用各种AI工具，有大量真实使用经验和踩坑经历。你的文章有信息增量，不是网上能搜到的泛泛而谈。
    ...
    if args.publish:
        ...
        try:
            ...
            if infographic_data:
                ...
                if png_path:
                    # 把插图插入到文章内容中
                    img_html = infographic_img_tag(article['slug'])
                    # 插在第一个 H2 之前或内容中部
                    content = article.get('content', '')
                    h2_idx = content.find('\n## ')
                    if h2_idx > 0:
                        article['content'] = content[:h2_idx] + '\n\n' + img_html + content[h2_idx:]
                    else:
