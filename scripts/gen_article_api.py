...
# ===== MiniMax 官方（M2.7）=====
MX_API_KEY = os.getenv("MINIMAX_API_KEY", "")
MX_BASE_URL = os.getenv("MINIMAX_BASE_URL", "https://api.minimaxi.com/anthropic")
MX_MODEL = "MiniMax-M2.7"

# 默认使用 M2.7（更快更强）
ACTIVE_MODEL = os.getenv("ARTICLE_MODEL", "m25")  # m27 或 m25
MAX_TOKENS = 8000
TEMPERATURE = 0.8
...
ARTICLE_TYPES = {
    'A': {
        'label': '数据驱动分析型',
        'description': '基于官方文档和公开数据对比分析AI工具',
        'structure': '开头 + 为什么写这篇 + 3个分析段落 + 对比表格 + 潜在限制与误区 + FAQ(3个问答) + 总结',
        'requirements': [
            '必须有基于来源的具体数据（标注出处链接）',
    ...
    'B': {
        ...
        'structure': '开头 + 为什么写这篇 + 3个对比段落 + 评分表格 + 潜在限制与误区 + FAQ(3个问答) + 总结',
        'requirements': [
    ...
    'D': {
        ...
        'structure': '开头 + 前置准备 + 3-4个步骤段落 + 常见问题与警告 + FAQ(3个问答) + 总结',
        'requirements': [
            '步骤具体到"点击哪个按钮""在哪找到这个选项"（基于官方文档）',
            '有常见问题和官方文档中的使用限制',
            '有基于官方教程的操作示例（不是抽象描述）',
            '内容简洁有力，不啰嗦，写完觉得没啥可补充的就收',
        ],
...
2. 数据要具体且有来源，禁止"显著提升""大幅优化"等模糊描述。必须标注出处，如"根据OpenAI官方博客，响应时间缩短了约57%"，而不写裸的"缩短57%"
3. 适当使用口语化表达："说实话""这一点很关键""但问题在于""别小看这个"
4. 标题要有信息增量，让人一看就知道这篇文章有什么不一样
5. 每段开头不能都是同一个句式，要有变化
6. 禁止每段都用总结性句子结尾，让人看着像教科书
7. **禁止伪造体验声明：不写"我测了""我踩了个坑""我用了X天后发现""经我实测"等——改用"根据官方文档""公开基准测试显示""数据库统计表明""交叉比对发现"**
"""

# ===== 禁止的内容类型 =====
BANNED_CONTENT_TYPES = """
## 禁止的内容类型（绝对不能写）
1. 纯盘点列举（"10个最好的AI工具"——没有分析深度的罗列就是水文章）
2. 官网信息搬运（功能介绍堆砌，没有独立观点）
3. 无观点的中立比较（"各有优劣，看你需求"）
4. 没有来源的数据声明（"提升70%"但没有标注出处）
5. 伪造的体验声明（"我花了两周测试""经我实测""我用了X天后发现"——你根本没测过）
6. 重复已有角度的内容（如果网上已经有100篇类似的，你的角度必须不同）
"""

# ===== 质量检查清单 =====
QUALITY_CHECKLIST = """
## 质量检查清单（生成文章前自问自检）
- [ ] 这篇文章有没有基于数据分析的独家见解？（交叉比对、统计结论、决策框架）
- [ ] 有没有具体数据且标注了来源？（官方文档链接、公开基准测试、数据库统计）
- [ ] 结论是否明确，不是模棱两可？（推荐X而不是"看需求"）
- [ ] 是否有结构化表格方便读者理解？（至少1个Markdown表格）
- [ ] 内链是否自然，不是硬塞的？（3-5个工具页链接）
- [ ] 措辞是否自然，没有AI腔？（检查禁用词汇表）
- [ ] 是否包含FAQ部分？（3-5个问答）
- [ ] 是否有"潜在限制与常见误区"段落？（基于文档分析，不是编造的踩坑故事）
- [ ] 是否有"为什么写这篇"段落？（展示分析动机和数据来源背景）
- [ ] 是否引用了1-2个外部权威来源？（官方文档、知名评测、行业报告，标注链接，增强GEO跨平台信任）
- [ ] ❌ 是否出现了禁止的伪体验句式？（"我测了""经我实测""我用了X天后""踩了个坑"等——全部删除）
"""

# ===== 主题库（按类型，每种7个） =====
TOPIC_POOL = {
    'A': [
        "Cursor vs Claude Code：基于官方基准测试的2026年AI编程工具深度对比",
        "Gemini 3 Pro处理200万token超长文档：官方基准与社区反馈的综合分析",
        "DeepSeek V3.2免费API成本分析：官方定价与用量数据解读",
        "Midjourney V7 vs Flux Pro vs 可灵：官方数据与用户反馈交叉比对",
        "用AI工具从零建站：5款工具的官方文档对比与选择指南",
        "ChatGPT Plus vs Claude Pro vs DeepSeek：定价、功能、使用限制全方位对比分析",
        "5款AI编程工具对比：基于公开基准测试的分维度分析",
    ],
    'B': [
        "Claude Code vs Cursor Pro：2026年AI编程工具到底选哪个（深度对比分析）",
        "DeepSeek V3.2 vs ChatGPT GPT-5.4：中文场景谁更强？基于公开基准测试",
        ...
        "Perplexity vs 秘塔 vs 博查：AI搜索引擎中文效果对比分析",
    ],
    'C': [
        "2026年完全免费的AI工具清单（持续更新，收录70+款）",
...
def make_tool_link(slug, name):
    """生成工具内链Markdown"""
    return f"[{name}](https://www.aitoollab.cn/tools/{slug}/)"


# ===== API客户端初始化 =====
# 硅基流动客户端（M2.5）
sf_client = OpenAI(api_key=SF_API_KEY, base_url=SF_BASE_URL)
# MiniMax官方客户端（M2.7，Anthropic兼容）
mx_client = anthropic.Anthropic(api_key=MX_API_KEY, base_url=MX_BASE_URL)

def call_api(system_prompt, user_prompt, max_retries=3):
    """调用API生成文章，支持M2.5(硅基流动)和M2.7(MiniMax官方)

    M2.7更快(5秒 vs 23秒)，字数更多(4200+ vs 2500+)，优先使用。
    """
    ...
    for attempt in range(max_retries):
        try:
            if model == "m27":
                # MiniMax-M2.7 (Anthropic SDK)
                resp = mx_client.messages.create(
                    model=MX_MODEL,
                    max_tokens=MAX_TOKENS,
                    temperature=TEMPERATURE,
                    system=system_prompt + "\n\n⚠️ 重要：禁止输出任何思考过程（Thinking Process），直接输出正文内容。",
                ...
                # 硅基流动 MiniMax-M2.5 (OpenAI SDK)
                resp = sf_client.chat.completions.create(
                    model=SF_MODEL,
                    messages=[
...
def quality_check(article):
    ...
    # 4. 潜在限制与误区检查
    if any(kw in content for kw in ['限制', '误区', '注意', '别犯', '不适合', '常见问题']):
        passes.append("潜在限制与误区: 有")
    else:
        warnings.append("潜在限制与误区: 缺少此部分")
...
def generate_article(topic, article_type, existing_titles, seo_keywords=None):
    ...
    system_prompt = f"""你是AI数据分析师，为网站www.aitoollab.cn撰写评测报告。你的信息来源：
1. 官方文档和公开基准测试（必须标注来源链接）
2. 本站收录的AI工具数据库（可引用统计结论，如"本类38款工具中免费仅占42%"）

## 核心原则：不做重复信息，做别人没有的分析

Google不缺信息罗列，缺的是有分析深度的内容。每篇文章必须通过"增量测试"：
- 这篇文章有没有基于数据得出的独家分析？（交叉比对、统计结论、决策建议）
- 如果用ChatGPT能生成一样的，这篇内容就没有存在价值
- **所有具体数字必须有据可查。不确定就写"以官方数据为准"，禁止编造精确百分比/时长/金额**

## 禁止的体验声明（绝对不写）
- "我花了X天/周/月测试" "我跑了X次" "我充值/订阅/买了" "经我实测" "我用了X天后发现"
- 改为："根据官方文档" "公开基准测试显示" "本站数据库统计" "多方信息交叉比对"
- **你的独有价值不是你"用过"，而是你"分析过"——交叉比对多方数据比单一个体体验更全面**

## 你的写作风格
- 像朋友在聊天，不像在写报告
- 有明确立场，敢于说"推荐用X"而不是"各有优劣"
- 数据具体且有来源："根据官方博客(链接)，响应时间缩短了约57%" 而非 "缩短57%"
- 适当口语化："说实话""这一点很关键""别小看这个"
- **禁止的角色口吻：** "我测了一下""我踩了个大坑""没想到""我当初"——你没有真正使用过，不要假装用了
...
格式：[工具名](https://www.aitoollab.cn/tools/工具slug/)
本篇可用内链工具：{link_md}
...
[开头：首句将作为页面 meta description 被搜索引擎展示。首句需含核心钩子+文章价值，独立成句能勾住搜索用户点击。后续自然展开补充，不需刻意控制字数。若整段超过约150中文字，精简后半段冗余即可（首句不变）。]

## 为什么写这篇
[分析动机和数据背景，展示研究视角。例如"最近几款工具同时发布了大版本更新，但官方数据差异很大，所以我们基于公开基准测试和文档做一次系统比对"——自然引出你为何做这个分析]
...
## 潜在限制与常见误区
[基于官方文档和公开信息的深度分析：哪些功能宣传得很强但实际使用中有客观限制（根据文档明确写了的使用上限、兼容性要求等）？哪些场景不适合？数字必须有来源链接]

## FAQ
[3-5个读者最可能问的问题，答案简洁有信息量。Q用粗体]

## 参考来源
[引用1-2个外部权威来源，如官方文档、知名评测、行业报告。格式：- [来源名称](URL)。这是GEO的核心——让AI看到你的内容引用权威信息]

## 总结
[明确结论+具体推荐+下一步建议。不是"各有优劣"，是"如果你是XX，推荐用XX"]"""

    if article_type == 'A':
        user_prompt = f"""写一篇数据驱动分析文章，主题：{topic}

核心要求：基于官方文档和公开基准测试进行分析。所有数字必须标注来源（如"根据OpenAI官方博客"）。禁止伪造个人使用体验。你的价值不是"我用过"，而是你交叉比对多方数据后得出的分析——这比单一个体的体验更全面。
{structure}
...
## 📊 对比数据块（如果文章涉及工具对比，输出JSON；教程/清单类可省略）
```json
{
  "title": "对比标题（15字以内）",
  "tools": [{"name":"工具名","sub":"厂商·型号","emoji":"🤖","badges":[["标签","type"]]}],
  "rows": [{"label": "维度", "type": "score|text|tags|scene", "values": [...]}]
}
```
（type=score用0-10数字，type=tags用{"yes":[],"no":[]}，type=text/scene用字符串）
tools固定3个，rows 4-6行。

不需要输出slug、日期等元数据。只输出Markdown文章+关键词块+对比数据块。"""

    elif article_type == 'B':
        user_prompt = f"""写一篇观点对比型文章，主题：{topic}

核心要求：必须给出明确的推荐结论。读者看完要知道"该选哪个"，而不是"各有优劣你自己想"。
每个工具要有具体的分维度评分数据（基于公开基准测试或官方文档）。禁止伪造个人测试数字。
{structure}
...
核心要求：基于官方文档和使用教程，像朋友手把手教新手。步骤要具体到"点击左上角的XX按钮""在设置里找到XX选项"。
可以指出官方文档中容易踩坑的地方（如某些功能的使用上限、兼容性限制），但要标注信息来源。
{structure}
...
不需要输出slug、日期等元数据。只输出Markdown文章+关键词块+对比数据块。"""
    ...
    model_label = "M2.7" if ACTIVE_MODEL == "m27" else "M2.5"
    print(f"  调用 {model_label} 生成文章...")
    print(f"  主题: {topic}")
    ...
    # ===== 后处理 =====
    # 0. 提取关键词块和对比数据块（在去AI味和标题提取之前）
    main_keyword, longtail_keywords = extract_seo_keywords(content)
    # 提取对比数据 JSON（用于信息图生成）
    infographic_data = None
    infographic_match = re.search(r'```json\s*\n({[\s\S]*?"tools"[\s\S]*?"rows"[\s\S]*?})\n```', content)
    if infographic_match:
        try:
            infographic_data = json.loads(infographic_match.group(1))
        except:
            pass
    # 去掉关键词块和信息图数据块（不放入正文）
    ...
    article = {
        ...
        "infographic": infographic_data,  # 对比信息图数据（可能为None）
    }

    if infographic_data:
        print(f"  📊 检测到对比数据块 ({len(infographic_data.get('rows', []))} 行)，可用于生成信息图")
...
def main():
    ...
    parser.add_argument('--model', type=str, choices=['m25', 'm27'], default=None, help='选择模型: m25=硅基流动MiniMax-M2.5(默认), m27=MiniMax-M2.7')
    ...
    model_name = "MiniMax-M2.7 (官方)" if ACTIVE_MODEL == "m27" else "MiniMax-M2.5 (硅基流动)"
    print(f"\n{'='*60}")
    ...
    print(f"内链数: {article['content'].count('](https://www.aitoollab.cn/tools/')}")
    ...
    if args.publish:
        ...
        print("\n[2/5] 生成正文插图")
        try:
            from gen_article_infographic import generate as gen_infographic, img_tag as infographic_img_tag, extract_from_content
            infographic_data = article.get('infographic') or extract_from_content(article.get('content', ''))
            if infographic_data:
                png_path = gen_infographic(infographic_data, article['slug'])
                if png_path:
                    # 信息图PNG已生成，由 build.py 统一在标记位置（或文末兜底）插入，避免重复
                    print(f"  ✅ 信息图PNG已生成（build.py统一插入）")
            else:
                print(f"  ℹ️  无对比数据，跳过插图")
        except ImportError:
            print(f"  ℹ️  插图模块未就绪，跳过")
        except Exception as e:
            print(f"  ⚠️ 插图失败: {e}")
        ...
        print(f"   Slug: {article['slug']}")
        print(f"   URL: https://www.aitoollab.cn/articles/{article['slug']}/")
        print(f"{'='*60}")
    else:
