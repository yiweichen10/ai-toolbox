...
def check_tool_health(tool):
    ...
    h2s = [l.strip() for l in content.split("\n") if l.strip().startswith("## ") and not l.strip().startswith("### ")]
    has_src = "来源" in content or "Source" in content
    ...
    return {
        ...
        "h2_count": len(h2s),
        "has_src": has_src,
        "content_blocked": bool(tool.get("content_blocked", False)),
    }
...
def run_check():
    ...
    # 2b. format_shrink 环比检测（对比上月报告，标记结构骤降的工具）
    print("\n--- 结构环比检测（vs 上月）---")
    shrink_issues = []
    prev_tool_csv = None
    # 找最近一份历史工具报告（排除今天）
    today_str = datetime.now().strftime("%Y-%m-%d")
    prev_files = sorted(REPORT_DIR.glob("tool_health_*.csv"))
    for f in prev_files:
        if today_str not in f.name:
            prev_tool_csv = f  # 取最新一份历史报告
    if prev_tool_csv:
        prev_stats = {}
        with open(prev_tool_csv, "r", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                prev_stats[row["slug"]] = row
        for r in tool_results:
            slug = r["slug"]
            p = prev_stats.get(slug)
            if not p:
                continue
            parts = []
            try:
                prev_h2 = int(p.get("h2_count", 0) or 0)
                prev_len = int(p.get("content_length", 0) or 0)
                if r["h2_count"] < prev_h2 - 2:
                    parts.append(f"H2骤降{prev_h2}->{r['h2_count']}")
                if prev_len > 300 and r["content_length"] < prev_len * 0.5:
                    parts.append(f"字数缩水{prev_len}->{r['content_length']}")
                if p.get("has_faq") == "True" and not r["has_faq"] and r["h2_count"] >= 3:
                    parts.append("FAQ丢失")
                if p.get("has_test_data") == "True" and not r["has_test_data"] and r["h2_count"] >= 3:
                    parts.append("实测数据丢失")
                if p.get("has_src") == "True" and not r["has_src"] and r["h2_count"] >= 3:
                    parts.append("来源丢失")
            except (ValueError, TypeError):
                pass
            if parts:
                shrink_issues.append((slug, parts))
        if shrink_issues:
            print(f"  ⚠️ 发现 {len(shrink_issues)} 个工具结构骤降（对比 {prev_tool_csv.name}）:")
            for slug, parts in sorted(shrink_issues, key=lambda x: -len(x[1]))[:15]:
                print(f"    {slug[:35]:<35} | {'; '.join(parts)}")
        else:
            print("  ✅ 无结构骤降工具")
    else:
        print("  （无历史报告可对比）")

    # 2c. content_blocked 待人工复核标记
    blocked = [r for r in tool_results if r["content_blocked"]]
    if blocked:
        print(f"\n  ⚠️ {len(blocked)} 个工具 content_blocked=True（apply 防护拦截的修正文，待人工复核）:")
        for r in blocked[:15]:
            print(f"    {r['slug'][:35]:<35} | verified_content 待复核")
    ...
    with open(tool_csv, "w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["slug", "name", "has_test_data", "has_faq", "has_src", "content_length", "h2_count", "content_blocked", "needs_update"])
        writer.writeheader()
