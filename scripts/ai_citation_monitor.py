...
# API配置 — 优先DeepSeek官方，SiliconFlow作为备用
# 从.env文件读取
...
# DeepSeek官方API（优先）
API_KEY = _env_vars.get("DEEPSEEK_API_KEY", os.getenv("DEEPSEEK_API_KEY", ""))
BASE_URL = _env_vars.get("DEEPSEEK_BASE_URL", os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com/v1"))
MODEL = _env_vars.get("DEEPSEEK_MODEL", os.getenv("DEEPSEEK_MODEL", "deepseek-v4-flash"))

# SiliconFlow备用（如果DeepSeek不可用）
SF_API_KEY = _env_vars.get("SILICONFLOW_API_KEY", os.getenv("SILICONFLOW_API_KEY", ""))
SF_BASE_URL = _env_vars.get("SILICONFLOW_BASE_URL", os.getenv("SILICONFLOW_BASE_URL", "https://api.siliconflow.cn/v1"))
SF_MODEL = "deepseek-ai/DeepSeek-V4-Pro"

# 自动切换：DeepSeek失败则尝试SiliconFlow
API_PROVIDERS = []
if API_KEY:
    API_PROVIDERS.append({"name": "DeepSeek官方", "key": API_KEY, "url": BASE_URL, "model": MODEL})
if SF_API_KEY:
    API_PROVIDERS.append({"name": "SiliconFlow", "key": SF_API_KEY, "url": SF_BASE_URL, "model": SF_MODEL})
...
def query_ai(prompt, timeout=120, max_retries=2):
    """调用AI API查询（自动切换provider，加重试+长超时）"""
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    import time
                    wait = attempt * 5
                    print(f"    重试 {attempt}/{max_retries}（等待{wait}秒）[{provider['name']}]...")
                    time.sleep(wait)
                else:
                    print(f"    {provider['name']} 全部重试失败: {e}")
                    continue  # 尝试下一个provider
    return "[ERROR] 所有API provider均失败"
...
def run_monitor():
    ...
    print(f"查询平台: {API_PROVIDERS[0]['name'] if API_PROVIDERS else 'N/A'} ({API_PROVIDERS[0]['model'] if API_PROVIDERS else 'N/A'})")
    print(f"查询数量: {len(MONITOR_QUERIES)}")
    print(f"备用平台: {', '.join(p['name'] for p in API_PROVIDERS[1:]) if len(API_PROVIDERS) > 1 else '无'}")
    print("=" * 60)

    if not API_PROVIDERS:
        print("[ERROR] 未找到可用的API provider，请检查 .env 配置")
        return
