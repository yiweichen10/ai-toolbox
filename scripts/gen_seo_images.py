...
def _try_screenshot_once(html, output_path, width, height):
    """单次截图尝试，成功返回True。遇到playwright-cli残留/竞态时自动清理重试。"""
...
def generate_image(html, output_path, width=1200, height=630, max_retries=2):
    """Playwright 截图生成 OG 图片，支持竞态重试。
    
    连续生成多张OG图时，playwright-cli close 可能未完全释放浏览器，
    导致下张截图失败。遇到失败时强制清理残留进程后重试。
    """
    for attempt in range(max_retries + 1):
        if attempt > 0:
            # 强制杀死可能残留的 playwright-cli 进程
            try:
                subprocess.run('taskkill /F /IM playwright-cli.exe 2>nul', shell=True, capture_output=True, timeout=5)
            except:
                pass
            time.sleep(1.5)
            print(f"  [OG] 第{attempt}次重试...")
        if _try_screenshot_once(html, output_path, width, height):
            return True
    return False
