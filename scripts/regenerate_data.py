...
def generate_ranking_data():
    ...
    def calc_score(tool):
        ...
        hot_score = min(40, math.log10(max(visits, 1)) * 5.7)
    ...
    def tool_entry(t, rank):
        return {
            ...
            "score": calc_score(t),
            "scores": {
                "hot": min(40, math.log10(max(parse_visits(t.get('visits', '')), 1)) * 5.7),
    ...
    # 1. 综合热度榜
    all_sorted = sorted(tools, key=lambda t: (calc_score(t), parse_visits(t.get('visits', ''))), reverse=True)
    rankings.append({
        "type": "special",
        "id": "overall",
    ...
    free_sorted = sorted(free_tools, key=lambda t: (calc_score(t), parse_visits(t.get('visits', ''))), reverse=True)
    rankings.append({
        "type": "special",
        "id": "free",
    ...
    value_sorted = sorted(value_tools, key=lambda t: (calc_score(t) / (1 + (0 if '免费' in str(t.get('price', '')) else 1)), parse_visits(t.get('visits', ''))), reverse=True)
    rankings.append({
        "type": "special",
        "id": "value",
    ...
    for cat in CATEGORIES:
        ...
        if len(cat_tools) == 0:
            continue
        cat_sorted = sorted(cat_tools, key=lambda t: (calc_score(t), parse_visits(t.get('visits', ''))), reverse=True)
        slug_map = {
