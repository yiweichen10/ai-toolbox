# render_live.py — Live 仪表盘与区块
# 模块9：从 build.py 拆分（2026-08-24）
import os
import re
import random
import json
...
from build_lib.render_tool import (
    make_tool_card_html, ensure_og_image,
)
...
def build_live_page(live_data, page_config, all_tools, articles):
    ...
    if len(meta_desc) < 115:
        _live_desc_suffix = f"AI工具宝箱提供{page_title}功能，涵盖多维度数据分析，帮助用户快速了解AI工具市场格局和趋势变化，支持按分类、价格与热度筛选对比。数据每日自动更新、来源可溯源，覆盖{build.TOOL_COUNT}款主流AI工具。"
    ...
    footer = '<footer class="footer"><p>&copy; ' + str(build.BUILD_YEAR) + ' AI工具宝箱 · 每日精选优质AI工具 · 更新于 ' + _ldt.now().strftime('%Y-%m-%d %H:%M') + ' · ' + build.ICP_BEIAN + '</p></footer>'
