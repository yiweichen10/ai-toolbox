...
"""
══════════════════════════════════════════════════════════════════════════
文章 meta description 产出规范（2026-07-24 定稿，权威定义见 build.py _get_article_description）
本脚本每篇文章配置里的 "description" 字段按此标准手写：
  1. 首句（前 ~70–80 字）必须含【钩子 + 核心价值】，独立成句能勾人
     —— 用户 SERP 只看到这一截（桌面中文约 70–100 字可见，移动约 40–55 字）
  2. 全文自然写完，不强行压缩、不凑字数
  3. 仅当全文超过约 150 中文字时，回头精简后半段冗余（首句钩子不动）
  4. 不堆砌关键词、不重复、准确概括该页内容
  5. 每篇 description 唯一
依据（Google Search Central 官方文档）：meta description 不是排名因素、无长度硬上限、
     按设备像素宽度截断；由长串关键字组成的描述不太可能被采用为摘要。
══════════════════════════════════════════════════════════════════════════
