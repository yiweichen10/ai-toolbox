...
14. **禁止「兜底」掩盖缺陷（2026-09-14 用户拍板，最高优先级之一）**：
    - 上游或中间产物缺字段/取值不合法时，一律**在入口处拒收 + 打印缺陷明细 + 让链路停下等人分析**。
      **禁止**用默认值、关键词猜测、静默跳过、try/except 吞错等方式"填上继续跑"。
    - 理由：兜底把**显性缺陷变成隐性错误**——错在下游出现、且没人知道；用户要的是
      "在需要兜底之前就把流程做对"。
    - 要求：① 不合格输入不许进门（拒收后名额由其他合格项补位，**如实少发不凑数**）；
      ② 门禁里的字段契约必须 **FAIL 阻断**，不是打印警告；
      ③ 存量坏数据由**人读过内容后判定写回**（逐文件 `.bak` + 写后读回校验），不许用脚本猜值；
      ④ 采集/校验脚本要提供 `--audit-*` 这类**只报告不改数据**的清单命令，供分析根因用。
    - 出错时的正确姿势（顺序不可颠倒）：**先取证定位根因（谁写坏的 + 谁放行的，两侧都要查）
      → 判断根因类型 → 只修根因 → 复现验证 → 再重跑对应段**。
      **禁止**为了"把任务跑完"而绕过错误、加兜底、盲目重试或从头全量重跑。
    - 反例（2026-09-14）：上游 `category: null` → 第一版修法是加关键词猜测兜底 + 渲染层 `or '资讯'`
      + 事后补值脚本，被用户否掉；正解是采集端拒收 + 契约门禁 FAIL + 人工判定存量值。
...
### 配图纪律补充：截图必须验"关键元素在视口内"（2026-09-14）

- 教训：本轮拍 4 张官方页截图，先拍的 2 张"看着有 200 多 KB"其实**关键内容在首屏之外**
  （platform.moonshot.cn 的价格卡在 y=1463，kimi.com/blog/kimi-k3 的模型参数在 y=1810），
  直接用会得到"标题在、要讲的东西不在"的废图。
- 做法：截图后**复现同一滚动位置**，对每张图要展示的关键文本调 `bounding_box()`
  断言 `0 <= y <= 视口高-20`，不满足就调视口高/滚动量重拍；4 张图逐张过一遍再引用。
- 同样适用于懒加载图：线上验收要**逐屏下滚**触发 `loading="lazy"`，只滚一次到底会误判"图没渲染"。
...
**顺带修**：① 未提交改动提示块原放在 `commit` **之前**，会把刚 staged 的文件列进来、并谎称
"不会上线"（实际已在本次 commit 内）——已移到 `commit` **之后**，语义修正为"除本次提交外的残留改动"。
② `cms.html`（`gen_cms.py` 产物、已被 git 跟踪）此前不在白名单 → 每次 `gen_cms` 后永久挂在"未提交"里、
改动无法回滚，已纳入白名单。


## 2026-09-11 deploy.sh 静默死亡两坑（Windows curl rc=23 + set -e 吞掉重试）——部署必读

当天新加的 `images/og/` 段**首跑即把整个部署脚本杀死在 [2/4] 段末**：退出码 23、日志无任何报错、
`.deploy_last_result.json` 永久停在 `RUNNING`。

**坑 1：Windows Git Bash 下 `curl -o /dev/null` 恒返回 rc=23**
- 实测：`_http=$(curl -s -o /dev/null -w '%{http_code}' URL)` → `rc=23`（Failed writing body），
  **但 `%{http_code}` 仍正确拿到 200**。连跑 3 次全 23，是确定性行为，非网络抖动。
- 致命点：脚本开头 `set -e`，**`var=$(cmd)` 赋值语句会继承 cmd 的退出码** → rc=23 直接终止脚本。
  表现 = 部署在 [2/4] 段末静默死掉，`[3/4]` 重载 Nginx、`[4/4]` git 提交全不执行，却毫无错误提示。
- 规则：**`set -e` 脚本里 curl 校验线上资源必须写 `|| true`，成败只认 `$_http`，不认 curl 退出码。**
  （已修 infographics 段与 og 段共 2 处；infographics 段自 08-21 起就带此隐患，只是恰好没触发。）

**坑 2：`set -e` 让"失败重试"变成死代码**
- 原写法：`tar ... | ssh ...` 换行后 `_rc=$?`。管道失败时 **`set -e` 在管道那一行就退出脚本**，
  下一行的 `_rc=$?`、重试提示、`rollback_deploy` 永远不会执行——重试逻辑形同不存在。
- 修法：用 `set +e` / `set -e` 包住管道再取 `_rc`（og 段、infographics 段均已修）。
- 通用口径：**`set -e` 脚本里，任何"先执行、再看退出码"的写法都必须 `set +e` 包住或 `|| _rc=$?`**，
  否则失败分支永远不可达，事故只会以"静默死亡"的形式表现出来。

**排障口径（本次靠它 3 分钟定位）**：`RUNNING` + 日志尾部停在某段 → **不要盲目重跑**，
先看尾部停在哪个子命令，再手动复现该子命令的退出码（如单独跑那条 curl 看 rc），
根因几乎立刻现形。
